...
interface KPI { totalRevenue: number; totalOrders: number; avgOrderValue: number; totalCustomers: number; }
interface TeamMember { id: string; employee_code: string; name: string; role: string; parent_id: string | null; revenue: number; orders: number; }
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (team.success) {
        ...
        team.data.filter((t: TeamMember) => t.role === 'ASM').forEach((t: TeamMember) => { exp[t.id] = true; });
        setExpandedASM(exp);
      }
      setLoading(false);
    ...
    <>
      ...
      {kpi && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <KPICard icon={<DollarSign className="w-5 h-5" />} label="Tổng Doanh thu" value={formatVND(kpi.totalRevenue)} suffix="đ" color="from-indigo-500 to-blue-600" />
          <KPICard icon={<ShoppingCart className="w-5 h-5" />} label="Tổng Đơn hàng" value={kpi.totalOrders.toString()} suffix="đơn" color="from-emerald-500 to-green-600" />
          <KPICard icon={<TrendingUp className="w-5 h-5" />} label="Trung bình / Đơn" value={formatVND(kpi.avgOrderValue)} suffix="đ" color="from-amber-500 to-orange-600" />
          <KPICard icon={<Users className="w-5 h-5" />} label="Khách hàng Active" value={kpi.totalCustomers.toString()} suffix="KH" color="from-pink-500 to-rose-600" />
        </div>
      )}
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyRevenue} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                ...
                <Tooltip formatter={(value: any) => [`${Number(value).toLocaleString('vi-VN')} đ`, 'Doanh thu']} contentStyle={{ borderRadius: '10px', border: '1px solid #e5e7eb', fontSize: 13 }} />
                <Bar dataKey="revenue" fill="url(#barGrad1)" radius={[6, 6, 0, 0]} />
                <defs><linearGradient id="barGrad1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#6366f1" /><stop offset="100%" stopColor="#818cf8" /></linearGradient></defs>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </ChartCard>

        <ChartCard title="🥧 Cơ cấu theo nhóm hàng">
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={revenueByCategory} cx="50%" cy="45%" innerRadius={55} outerRadius={90} paddingAngle={4} dataKey="value"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <ChartCard title="">
          <h2 className="text-base font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2 -mt-4">
            <Briefcase className="w-5 h-5 text-indigo-500" /> Doanh thu đội ngũ bán hàng
          </h2>
          <div className="space-y-1 max-h-[400px] overflow-y-auto">
            {asmList.map(asm => (
              <div key={asm.id}>
                <button onClick={() => setExpandedASM(p => ({ ...p, [asm.id]: !p[asm.id] }))} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors text-left">
                ...
                {expandedASM[asm.id] && ssList.filter(ss => ss.parent_id === asm.id).map(ss => (
                  <div key={ss.id} className="ml-6">
                    <button onClick={() => setExpandedSS(p => ({ ...p, [ss.id]: !p[ss.id] }))} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors text-left">
      ...
      {/* Channel bar */}
      <ChartCard title="🏪 Doanh thu theo Kênh phân phối" className="mt-4">
            ...
            return (
              <div key={ch.name} className="flex-1">
                ...
                <div className="h-24 bg-gray-100 dark:bg-zinc-800 rounded-lg overflow-hidden flex items-end">
                  <div className="w-full rounded-lg transition-all duration-500" style={{ height: `${pct}%`, background: i === 0 ? 'linear-gradient(to top, #6366f1, #818cf8)' : 'linear-gradient(to top, #f59e0b, #fbbf24)' }} />
                </div>
              </div>
            );
          })}
